...
void allocationDynGraph(int nbBoitesV,int nbBoitesH,int nbLabels,int nbBoutons,int nbImages,int nbEntrees){
  pApp->nbBoitesV=nbBoitesV;
  pApp->boitesV=(BoiteVerticale*)malloc(pApp->nbBoitesV*sizeof(BoiteVerticale));
  pApp->nbBoitesH=nbBoitesH;
  pApp->boitesH=(BoiteHorizontale*)malloc(pApp->nbBoitesH*sizeof(BoiteHorizontale));
...
void liberationDynamique(){
  if(pApp->nbBoutons!=0) free(pApp->boutons);
  if(pApp->nbLabels!=0) free(pApp->labels);
  if(pApp->nbImages!=0) free(pApp->images);
  if(pApp->nbEntrees!=0) free(pApp->entrees);
  if(pApp->nbBoitesV!=0) free(pApp->boitesV);
  if(pApp->nbBoitesH!=0) free(pApp->boitesH);
}
...
void quitter(PWidget widget,gpointer pData){
  ...
  pApp=(Programme*)pData;
  //Creation de la boite de message
  pQuestion=gtk_message_dialog_new(GTK_WINDOW(pApp->fenetre),GTK_DIALOG_MODAL,
...
void nouvelleExp(PWidget pWidget,gpointer pData){
  ...
  pApp->boutons[2]=gtk_button_new_with_label("Generation par l'utilisateur");
  ...
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[0],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[1],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[2],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[3],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesH[0]),pApp->images[0],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesH[0]),pApp->boitesV[0],FALSE,FALSE,0);
  ...
  g_signal_connect(G_OBJECT(pApp->boutons[0]),"clicked",G_CALLBACK(generationAuto),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[1]),"clicked",G_CALLBACK(chargementGraph),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[2]),"clicked",G_CALLBACK(generationUtilisateur),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[3]),"clicked",G_CALLBACK(acceuil),(gpointer*) pApp);
}
...
void parametresGraph(PWidget pWidget,gpointer pData){
  ...
  sprintf(param1,"%dx%d",parametres.taille.hauteur,parametres.taille.largeur);
  ...
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[3],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesH[0]),pApp->boitesV[0],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[1]),pApp->labels[0],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[1]),pApp->labels[1],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[1]),pApp->labels[2],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[1]),pApp->labels[3],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesH[0]),pApp->boitesV[1],FALSE,FALSE,0);
  //creation du container pour mettre la premiere boite
  gtk_container_add(GTK_CONTAINER(pApp->fenetre),pApp->boitesH[0]);
  //affichage de la fenetre nouvellement creee
  gtk_widget_show_all(pApp->fenetre);
  ...
  if(jeuEnCours==0){
    g_signal_connect(G_OBJECT(pApp->boutons[0]),"clicked",G_CALLBACK(choixTailleExp),(gpointer*) pApp);
  }
  else{
	g_signal_connect(G_OBJECT(pApp->boutons[0]),"clicked",G_CALLBACK(modifTailleImp),(gpointer*) pApp);
  }
  g_signal_connect(G_OBJECT(pApp->boutons[1]),"clicked",G_CALLBACK(choixVitesseExp),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[2]),"clicked",G_CALLBACK(choixEspaceExp),(gpointer*) pApp);
  if(jeuEnCours==0){
	g_signal_connect(G_OBJECT(pApp->boutons[3]),"clicked",G_CALLBACK(acceuil),(gpointer*) pApp);
  }
  else{
	g_signal_connect(G_OBJECT(pApp->boutons[3]),"clicked",G_CALLBACK(pauseGraph),(gpointer*) pApp);
  }
}
...
void choixTailleExp(PWidget pWidget,gpointer pData){
  ...
  pApp=(Programme*)pData;
  //creation de la boite de dialogue
  pDialog=gtk_dialog_new_with_buttons("Taille de l'experience",GTK_WINDOW(pApp->fenetre),
				      GTK_DIALOG_MODAL,"20 par 20",REPONSE1,
...
void choixVitesseExp(PWidget pWidget,gpointer pData){
  ...
  pApp=(Programme*)pData;
  //creation de la boite de dialogue
  pDialog=gtk_dialog_new_with_buttons("Vitesse de l'experience",GTK_WINDOW(pApp->fenetre),
				      GTK_DIALOG_MODAL,"Lente",REPONSE1,
...
void choixEspaceExp(PWidget pWidget,gpointer pData){
  ...
  pApp=(Programme*)pData;
  //creation de la boite de dialogue
  pDialog=gtk_dialog_new_with_buttons("Espace de l'experience",GTK_WINDOW(pApp->fenetre),
				      GTK_DIALOG_MODAL,"Torique",REPONSE1,
...
void explications(PWidget pWidget,gpointer pData){
  ...
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[0],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->labels[0],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->labels[1],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->labels[2],FALSE,FALSE,0);
  ...
  g_signal_connect(G_OBJECT(pApp->boutons[0]),"clicked",G_CALLBACK(acceuil),(gpointer*) pApp);
}
...
void pauseGraph(PWidget pWidget,gpointer pData){
  ...
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[2],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[3],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[4],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesH[0]),pApp->labels[0],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesH[0]),pApp->boitesV[0],FALSE,FALSE,0);
  //creatoin du container pour la premiere boite
  gtk_container_add(GTK_CONTAINER(pApp->fenetre),pApp->boitesH[0]);
  //affichage de la fenetre principale et de ses composants
  ...
  g_signal_connect(G_OBJECT(pApp->boutons[0]),"clicked",G_CALLBACK(nouvelleExp),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[1]),"clicked",G_CALLBACK(sauvegardeGraph),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[2]),"clicked",G_CALLBACK(parametresGraph),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[3]),"clicked",G_CALLBACK(algo),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[4]),"clicked",G_CALLBACK(quitter),(gpointer*) pApp);
}
...
void algo(PWidget pWidget,gpointer pData){
  ...
  g_signal_connect(G_OBJECT(pApp->boutons[0]),"clicked",G_CALLBACK(pauseGraph),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[1]),"clicked",G_CALLBACK(quitter),(gpointer*) pApp);
...
void chargementGraph(PWidget pWidget,gpointer pData){
  ...
  pApp=(Programme*)pData;
  //Creation de la boite de dialogue
  pBoite=gtk_dialog_new_with_buttons("Chargement d'un fichier",GTK_WINDOW(pApp->fenetre),
				     GTK_DIALOG_MODAL,GTK_STOCK_OK,GTK_RESPONSE_OK,
				     GTK_STOCK_CANCEL,GTK_RESPONSE_CANCEL,NULL);
  //Creation de la zone de saisie
  pEntry = gtk_entry_new();
  gtk_entry_set_text(GTK_ENTRY(pEntry),"Saisissez le nom du fichier a charger");
...
void sauvegardeGraph(PWidget pWidget,gpointer pData){
  ...
  pApp=(Programme*)pData;
  //Creation de la boite de dialogue
  pBoite=gtk_dialog_new_with_buttons("Sauvegarde d'un fichier",GTK_WINDOW(pApp->fenetre),
				     GTK_DIALOG_MODAL,GTK_STOCK_OK,GTK_RESPONSE_OK,
				     GTK_STOCK_CANCEL,GTK_RESPONSE_CANCEL,NULL);
  //Creation de la zone de saisie
  pEntry = gtk_entry_new();
...
void generationAuto(PWidget pWidget,gpointer pData){
  ...
  pApp->labels[1]=gtk_label_new(intToCharMatrice(matriceG,parametres.taille));
  //insertion des elements dans les boites associees
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[0],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[1],FALSE,FALSE,0);
  gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boutons[2],FALSE,FALSE,0);
  ...
  g_signal_connect(G_OBJECT(pApp->boutons[0]),"clicked",G_CALLBACK(generationAuto),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[1]),"clicked",G_CALLBACK(nouvelleExp),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[2]),"clicked",G_CALLBACK(algo),(gpointer*) pApp);
}
...
void generationUtilisateur(PWidget pWidget,gpointer pData){
  ...
  if(generEnCours==0){
    ...
    if(jeuEnCours==0){
      ...
      matriceGener = (int**) malloc(((parametres.taille).hauteur)*sizeof(int*));
      for(i=0; i<((parametres.taille).hauteur); i++) {
		matriceGener[i] = (int*) malloc(((parametres.taille).largeur)*sizeof(int));
      }
      for(i=0; i<((parametres.taille).hauteur); i++) {
    ...
    pApp->labels[0]=gtk_label_new(intToCharMatrice(matriceGener,parametres.taille));
    pApp->labels[1]=gtk_label_new("Entrez les parametres de\nla souche a ajouter ou supprimer");
	...
	//insertion des elements dans les boites associees
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[1]),pApp->labels[1],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[2]),pApp->labels[2],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[2]),pApp->boutons[0],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[2]),pApp->boutons[1],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[2]),pApp->boutons[2],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[2]),pApp->boutons[3],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[2]),pApp->labels[5],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[3]),pApp->labels[3],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[3]),pApp->boutons[4],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[3]),pApp->boutons[5],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[3]),pApp->boutons[6],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[3]),pApp->boutons[7],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[3]),pApp->labels[6],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[4]),pApp->labels[4],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[4]),pApp->boutons[8],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[4]),pApp->boutons[9],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[4]),pApp->labels[7],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[5]),pApp->boutons[10],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[5]),pApp->boutons[11],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[6]),pApp->boutons[12],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[6]),pApp->boutons[13],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boitesH[1],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boitesH[2],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boitesH[3],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boitesH[4],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boitesH[5],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesV[0]),pApp->boitesH[6],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[0]),pApp->labels[0],FALSE,FALSE,0);
    gtk_box_pack_start(GTK_BOX(pApp->boitesH[0]),pApp->boitesV[0],FALSE,FALSE,0);
    //creatoin du container pour la premiere boite
    gtk_container_add(GTK_CONTAINER(pApp->fenetre),pApp->boitesH[0]);
    ...
    g_signal_connect(G_OBJECT(pApp->boutons[0]),"clicked",G_CALLBACK(moins10X),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[1]),"clicked",G_CALLBACK(moinsX),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[2]),"clicked",G_CALLBACK(plusX),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[3]),"clicked",G_CALLBACK(plus10X),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[4]),"clicked",G_CALLBACK(moins10Y),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[5]),"clicked",G_CALLBACK(moinsY),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[6]),"clicked",G_CALLBACK(plusY),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[7]),"clicked",G_CALLBACK(plus10Y),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[8]),"clicked",G_CALLBACK(moinsSouche),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[9]),"clicked",G_CALLBACK(plusSouche),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[10]),"clicked",G_CALLBACK(ajouter),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[11]),"clicked",G_CALLBACK(retirer),NULL);
    g_signal_connect(G_OBJECT(pApp->boutons[12]),"clicked",G_CALLBACK(nouvelleExp),(gpointer*) pApp);
    g_signal_connect(G_OBJECT(pApp->boutons[13]),"clicked",G_CALLBACK(validMatrice),NULL);
  }
  else{
	//modification des champs devant changer a chaque rappel de la fonction
    gtk_label_set_text(GTK_LABEL(pApp->labels[0]),intToCharMatrice(matriceGener,parametres.taille));
    sprintf(chaine,"%d",xGener);
...
void validMatrice(PWidget pWidget,gpointer pData){
  ...
  for(i=0; i<((parametres.taille).hauteur); i++) {
    for(j=0; j<((parametres.taille).largeur); j++) {
      if(matriceGener[i][j]>nbSouchesGener) nbSouchesGener=matriceGener[i][j];
    }
  }
...
void acceuil(PWidget pWidget,gpointer pData){
  ...
  g_signal_connect(G_OBJECT(pApp->boutons[0]),"clicked",G_CALLBACK(nouvelleExp),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[1]),"clicked",G_CALLBACK(parametresGraph),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[2]),"clicked",G_CALLBACK(explications),(gpointer*) pApp);
  g_signal_connect(G_OBJECT(pApp->boutons[3]),"clicked",G_CALLBACK(quitter),(gpointer*) pApp);
}
