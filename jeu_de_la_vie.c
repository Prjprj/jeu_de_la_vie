...
int* nbCellVoisinesEspaceTorique (int i, int j, int nbSouches, Souche *souche) {
	...
	for(k=0; k<nbSouches; k++) {
		...
		if ((i!=0) && (j!=0)) {
			...
			compteur = comptage(compteur, souche[k].mat[i-1][j-1], typeCell);
			compteur = comptage(compteur, souche[k].mat[i-1][(j+1)%largeur], typeCell);
			compteur = comptage(compteur, souche[k].mat[i][j], typeCell);
			compteur = comptage(compteur, souche[k].mat[i][j-1], typeCell);
			compteur = comptage(compteur, souche[k].mat[i][(j+1)%largeur], typeCell);
			compteur = comptage(compteur, souche[k].mat[(i+1)%hauteur][j], typeCell);
			compteur = comptage(compteur, souche[k].mat[(i+1)%hauteur][j-1], typeCell);
			compteur = comptage(compteur, souche[k].mat[(i+1)%hauteur][(j+1)%largeur], typeCell);
		}

		else {
			if((i==0) && (j!=0)) {
				compteur = comptage(compteur, souche[k].mat[hauteur-1][j], typeCell);
				compteur = comptage(compteur, souche[k].mat[hauteur-1][j-1], typeCell);
				compteur = comptage(compteur, souche[k].mat[hauteur-1][(j+1)%largeur], typeCell);
				compteur = comptage(compteur, souche[k].mat[i][j], typeCell);
				compteur = comptage(compteur, souche[k].mat[i][j-1], typeCell);
				compteur = comptage(compteur, souche[k].mat[i][(j+1)%largeur], typeCell);
				compteur = comptage(compteur, souche[k].mat[(i+1)%hauteur][j], typeCell);
				...
				if((i!=0) && (j==0)) {
					compteur = comptage(compteur, souche[k].mat[i-1][j], typeCell);
					compteur = comptage(compteur, souche[k].mat[i-1][largeur-1], typeCell);
					compteur = comptage(compteur, souche[k].mat[i-1][(j+1)%largeur], typeCell);
					compteur = comptage(compteur, souche[k].mat[i][j], typeCell);
					compteur = comptage(compteur, souche[k].mat[i][largeur-1], typeCell);
					compteur = comptage(compteur, souche[k].mat[i][(j+1)%largeur], typeCell);
					compteur = comptage(compteur, souche[k].mat[(i+1)%hauteur][j], typeCell);
					compteur = comptage(compteur, souche[k].mat[(i+1)%hauteur][largeur-1], typeCell);
					compteur = comptage(compteur, souche[k].mat[(i+1)%hauteur][(j+1)%largeur], typeCell);
				}

				else {
					compteur = comptage(compteur, souche[k].mat[hauteur-1][j], typeCell);
					compteur = comptage(compteur, souche[k].mat[hauteur-1][largeur-1], typeCell);
					compteur = comptage(compteur, souche[k].mat[hauteur-1][(j+1)%largeur], typeCell);
					compteur = comptage(compteur, souche[k].mat[i][j], typeCell);
					compteur = comptage(compteur, souche[k].mat[i][largeur-1], typeCell);
					compteur = comptage(compteur, souche[k].mat[i][(j+1)%largeur], typeCell);
...
int* nbCellVoisinesEspaceCylindrique (int i, int j, int nbSouches, Souche *souche) {
						...
						if ((i==0) && (j==(largeur-1))) {
							...
							if ((i==(hauteur-1)) && (j==0)) {
								...
								if ((i==(hauteur-1)) && (j==(largeur-1))) {
									...
									if ((i==(hauteur-1)) && (j!=0) && (j!=(largeur-1))) {
										compteur = comptage(compteur, souche[k].mat[i][j], typeCell);
										compteur = comptage(compteur, souche[k].mat[i][j-1], typeCell);
										compteur = comptage(compteur, souche[k].mat[i-1][j-1], typeCell);
										compteur = comptage(compteur, souche[k].mat[i-1][j], typeCell);
										...
										if ((i!=0) && (i!=(hauteur-1)) && (j==(largeur-1))) {
											compteur = comptage(compteur, souche[k].mat[i][j], typeCell);
											compteur = comptage(compteur, souche[k].mat[i-1][j], typeCell);
											compteur = comptage(compteur, souche[k].mat[i-1][j-1], typeCell);
											compteur = comptage(compteur, souche[k].mat[i][j-1], typeCell);
	...
	for(i=0; i<taille.hauteur; i=i+1) {
		for(j=0; j<taille.largeur; j=j+1) {
			if (estCelluleVide(matriceG[i][j]) == TRUE){
				if ((valeur = celluleDoitNaitre(i, j, nbSouchesG, soucheG)) != 0) {
